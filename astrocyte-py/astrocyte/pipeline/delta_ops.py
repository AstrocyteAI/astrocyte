"""Delta operations for structured mental models (M21).

The LLM's job during a mental-model refresh is to emit a list of these
operations, each targeting an existing section (by id) or referencing a
position relative to one. :func:`apply_operations` validates and
applies each op in turn against a copy of the document; invalid ops
(unknown ``section_id``, out-of-range ``index``, malformed payloads)
are dropped with a debug-friendly reason — the document never gets
worse than its input.

Sections and blocks not mentioned by any op are physically copied
through unchanged — there is no LLM-mediated re-emission of unchanged
text, so prose drift is structurally impossible.

Why operations and not "output the new structured doc":

- "Output the new doc" still asks the LLM to *generate* every section's
  blocks, including ones it didn't intend to modify, which gives it
  the same opportunity to drift.
- Operations make the no-change case mechanical: zero ops → identical
  doc.
- Operations are auditable: each refresh produces a log of exactly
  what changed, useful for debugging the LLM's behaviour and
  explaining diffs.

Failure modes are by design conservative: an operation list that fails
to parse against the Pydantic schema, or an LLM that returns invalid
ops, results in zero changes — the document stays as-is. The
structure can only get better or stay the same per refresh, never get
worse.

Ported from Hindsight ``hindsight_api/engine/reflect/delta_ops.py``
...
_logger = logging.getLogger("astrocyte.pipeline.delta_ops")
...
class AddSectionOp(_OpBase):
    ...
    ``after_section_id`` is optional; when omitted the new section is
    appended at the end. ``new_id`` is optional; when omitted we
    slugify the heading and disambiguate against existing IDs.
    """
...
class ReplaceSectionBlocksOp(_OpBase):
    ...
    Used when most of a section's contents are stale and rebuilding it
    as a unit is clearer than emitting many block-level ops. The
    section's heading and id are preserved.
    """
...
class AppliedDelta(BaseModel):
    ...
    ``applied`` is a per-op summary (audit trail). ``skipped`` is a
    parallel list for ops the validator dropped, each carrying a
    ``reason`` so the refresh path can log why the LLM's output was
    rejected without spinning the cycle of "no idea what failed".
    """
...
def _op_summary(op: Operation) -> dict[str, Any]:
    ...
    Drops the verbose ``block`` / ``blocks`` payloads — the audit
    cares about which sections were modified, not what content was
    inserted. Use the full :class:`AppliedDelta.document` to inspect
    the new state.
    """
    data = op.model_dump()
...
def apply_operations(
    ...
    section, out-of-range index, name collision when adding a section)
    are skipped and recorded in ``skipped`` with a ``reason`` string.
    """
    ...
    def skip(op: Operation, reason: str) -> None:
        ...
        _logger.debug("delta_ops: skipping op %s", entry)
    ...
    for op in operations:
        ...
        if isinstance(op, InsertBlockOp):
            ...
            if op.index > len(section.blocks):
                skip(op, f"index out of range: {op.index} > {len(section.blocks)}")
                continue
