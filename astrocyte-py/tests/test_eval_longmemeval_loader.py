...
class TestSessionLevelRetention:
    ...
    def test_multiple_sessions_produce_separate_entries(self, tmp_path: Path) -> None:
        ...
        date_by_id = {e["session_id"]: e.get("session_date") for e in questions[0].conversation_context}
            ...
            [
                {
                    ...
                    "answer": "a",
                    "haystack_session_ids": ["s1"],
                    "haystack_sessions": [[{"role": "user", "content": f"turn {i}"} for i in range(n_turns)]],
                }
            ],
        )
        questions = load_longmemeval_dataset(path)
            ...
            [
                {
                    ...
                    "haystack_sessions": [
                        [{"role": "user", "content": "a"}, {"role": "user", "content": "b"}] for _ in range(5)
                    ],
                }
            ],
        )
...
# ---------------------------------------------------------------------------
# question_date wiring (Pass-4 bug fix)
# ---------------------------------------------------------------------------
#
# Bug: ``LongMemEvalQuestion`` had no ``question_date`` field and the loader
# never extracted it from the upstream record, so every relative phrase in a
# question (``"yesterday"``, ``"X days ago"``) was resolved against the
# bench's wall-clock — silently broken on a 2023-vintage dataset measured in
# 2026+. These tests pin the parse + propagation contract.


class TestQuestionDateParsing:
    def test_question_date_parsed_from_upstream_field(self, tmp_path: Path) -> None:
        path = _write_fixture(
            tmp_path,
            [
                {
                    ...
                    "question": "When did Alice last visit?",
                    "answer": "yesterday",
                    "question_date": "2024/01/15 (Mon) 10:00",
                    "haystack_sessions": [[{"role": "user", "content": "hi"}]],
                    "haystack_session_ids": ["s1"],
                    "haystack_dates": ["2024/01/14 (Sun) 09:00"],
                }
            ],
        )
        questions = load_longmemeval_dataset(path)
        assert len(questions) == 1
        q = questions[0]
        assert q.question_date is not None
        assert q.question_date.year == 2024
        assert q.question_date.month == 1
        assert q.question_date.day == 15
        assert q.question_date.hour == 10

    def test_question_date_none_when_field_absent(self, tmp_path: Path) -> None:
        """Records without ``question_date`` (older fixtures or bad data)
        load cleanly with ``question_date=None`` rather than raising —
        the bench then falls back to wall-clock anchoring (the legacy
        behavior, restored only when there's no better option)."""
        path = _write_fixture(
            tmp_path,
            [
                {
                    ...
                    "question": "What is X?",
                    "answer": "Y",
                    "haystack_sessions": [[{"role": "user", "content": "X is Y"}]],
                    "haystack_session_ids": ["s1"],
                }
            ],
        )
        questions = load_longmemeval_dataset(path)
        assert len(questions) == 1
        assert questions[0].question_date is None

    def test_question_date_none_on_unparseable_string(self, tmp_path: Path) -> None:
        path = _write_fixture(
            tmp_path,
            [
                {
                    ...
                    "question": "When?",
                    "answer": "yesterday",
                    "question_date": "garbage not a date",
