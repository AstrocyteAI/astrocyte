...
class TestVerifyPremise:
    ...
    async def test_supported_high_confidence_returns_supported_true(self):
        ...
        llm = _ScriptedLLM(
            ['{"supported": true, "confidence": 0.9, "evidence_ids": ["m1"], "rationale": "directly attested"}']
        )
    ...
    async def test_low_confidence_overrides_supported_true(self):
        ...
        llm = _ScriptedLLM(['{"supported": true, "confidence": 0.4, "evidence_ids": [], "rationale": "weak"}'])
        ...
        assert verdict.supported is False, "Confidence below threshold must override supported=True"
    ...
    async def test_no_recall_hits_yields_unsupported(self):
        ...
        llm = _ScriptedLLM(
            ['{"supported": false, "confidence": 0.95, "evidence_ids": [], "rationale": "no evidence in memory"}']
        )
