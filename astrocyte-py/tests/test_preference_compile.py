...
class TestPreferenceCompile:
    ...
    async def test_idempotent_on_existing_preferences(self) -> None:
        ...
        store = InMemoryMentalModelStore()
        provider = _provider(
            '{"preferences": [{"title": "Coffee", "content": "User prefers cortado.", "source_fact_ids": ["f1"]}]}'
        )
        facts = [
    ...
    async def test_caps_at_max_preferences(self) -> None:
        store = InMemoryMentalModelStore()
        prefs_json = ",".join(
            f'{{"title":"P{i}","content":"User prefers item-{i}.","source_fact_ids":["f{i}"]}}' for i in range(20)
        )
