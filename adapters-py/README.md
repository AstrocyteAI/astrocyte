...
- Includes integration tests that **skip** when the database is not running (local dev) and run in CI with Docker service containers (`.github/workflows/adapters-ci.yml`).
...
```bash
cd adapters-py/astrocyte-qdrant && uv sync --extra dev && uv run pytest
```
