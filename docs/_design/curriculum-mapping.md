...
| ------------------------------------ | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Context & Memory**                 | Governed memory and retrieval—not “RAG only”     | Core **retain** / **recall** / **reflect**; hybrid retrieval; Tier 1 vs Tier 2 providers ([Architecture framework §2](./architecture.md))            |
...
| **Runtime**                          | Orchestration, workers, sandboxes                | **Out of scope** — agent frameworks own the harness ([Architecture framework §1](./architecture.md))                                                 |
...
| **Harness vs context**                             | Agent loop vs what the model sees                      | Same split as **harness engineering vs context engineering** in [Architecture framework §1](./architecture.md)      |


## Suggested teaching hooks

- **Context & Memory week** (architect track): Read [Architecture framework](./architecture.md) §§1–3 and [Design principles](./design-principles.md); optional hands-on: Astrocyte + a Tier 1 adapter (for example PostgreSQL + pgvector) with a written **memory boundary** spec (banks, principals, recall strategies).
