...
5. **Unified consolidation scheduler (M49+ shape decision now)**: observation consolidation + mental-model refresh + wiki compile under one offline scheduler with value-based-forgetting hooks; the OKF-aligned `stale_after`/`status`/trust fields **proposed** in §6 would double as forgetting-policy inputs **once built — they do not exist today** (verified 2026-09-06: zero migrations define `stale_after`; the only `status` column in the schema is on `astrocyte_wiki_lint_issues`; no trust/actor field is persisted anywhere). Treat this as a dependency on §6, not an available input. Note: `retained_at`/`occurred_at` already constitutes TOKI-style bitemporality (arXiv:2606.06240) — adopt the vocabulary, build nothing. **Activation compile** (`compile(M, model_abi) → z`) hangs on this same scheduler; flags default OFF. SPI, Engram/Cartridge/Still negotiation, and the E-vs-C experiment are in [`m49-memory-activation.md`](m49-memory-activation.md) — docs only until after AML cycle 2.
