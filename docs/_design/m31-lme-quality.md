...
4. **Per-Q-type RRF arm weighting** — ✅ **IMPLEMENTED as M34 (intent-weighted RRF), then partially retracted by v015k/v015l evidence.** The mechanism shipped in full: `pipeline/intent_weights.py` (QueryIntent → per-channel weight table), `fact_recall(intent=...)` with `_rrf_fuse_fact_hits_weighted` (weight 0 mutes a channel), and the M31c `search_facts_keyword` primitive re-wired as an intent-gated 5th channel (FACTUAL boosts bm25 to 1.5; channel stays off when `intent is None`). See [m34-query-intent-routing.md](m34-query-intent-routing.md). **Bench verdict (v015k forensic)**: the weighting plumbing worked, but the regex *classifier* misrouted LME questions — 7/15 temporal-reasoning queries classified FACTUAL (temporal weight 0.3 killed TR's gain), SSP fell to UNKNOWN (no PREFERENCE intent exists in the taxonomy), SSA 15/15 UNKNOWN. M34-7 (v015l) removed intent from the bench fact_recall path, keeping per-fact-type segmentation only — which retained the LoCoMo wins (multi-hop +5.1pp, open-domain +3.2pp) while restoring LME. **Residual direction, if revisited**: the bottleneck is classifier quality, not fusion plumbing — a PREFERENCE intent + fixing the TR→FACTUAL precedence would need to land (and bench clean) before re-enabling intent in the fact_recall bench path. Note Hindsight ships NO intent classifier at 94.6% LME; treat further classifier investment as speculative. `recall_stage.py` retains `enable_intent_aware_recall` for the section-strategy/observation-injection layer, where it remains active. External supporting evidence for the philosophy (not mechanism): DeepSeek's Engram (arXiv 2601.07372).
