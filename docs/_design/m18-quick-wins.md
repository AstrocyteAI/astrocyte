...
6. **Retrieval-feature ceiling reached (M18b.2 finding).** Across M18b + M18b.2 we tested four retrieval-side features as candidates to add to the shipped B1-dp+RRF: B2 (directive_compile), B3 (spreading_activation), B4 (episodic_extract), and stacking combinations (B5 = B1+prompt, B1+B3). **None composed with shipped B1-dp+RRF for additional accuracy** — they were either null individually (B3/B4) or anti-composed when stacked. The pattern: every stacking attempt produced a per-category trade (one category +N, another category −N) that net out to combined ≤ best individual. The RRF fusion appears to have a saturation point: adding more parallel strategies dilutes cross-strategy agreement rather than reinforcing it. **M19 implication:** further accuracy work must target NEW pipeline stages (answerer-side prompt routing, pre-RRF gating filters, iterative recall), NOT additional RRF siblings.
