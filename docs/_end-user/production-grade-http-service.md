...
- [ ] **PostgreSQL reference stack:** Optional **[`astrocyte-postgres`](../adapters-storage-py/astrocyte-postgres/README.md)** plus PgQueuer; **[`docker-compose.yml`](../astrocyte-services-py/docker-compose.yml)** under **`astrocyte-services-py/`** can run API + Postgres. The default reference stack wires `vector_store: postgres`, `wiki_store: postgres`, wiki compile, entity resolution (via flat-table SQL CTEs), normalized temporal facts, and PgQueuer-backed async memory tasks. Add `graph_store: neo4j` only if you need cross-bank relationship queries via `graph_search()` / `graph_neighbors()`.
...
- [ ] **Structured logs (gateway):** Enable JSON access/service logging; set **`ASTROCYTE_LOG_FORMAT=json`** on **`astrocyte-gateway-py`** so access lines include **`request_id`**, **`method`**, **`path`**, **`status_code`**, **`duration_ms`**; responses echo **`X-Request-ID`**.
- [ ] **Structured logs (fields):** Include **request_id**, **principal** (hashed or opaque id if sensitive), **bank_id**, **route**, **latency**, **status** where applicable; define a **PII redaction** policy for log bodies.
- [ ] **Structured logs (ingest):** With the same **`ASTROCYTE_LOG_FORMAT`**, optional **`astrocyte.ingest`** packages emit JSON lines for lifecycle and transport errors (**`astrocyte.ingest.logutil`**).
