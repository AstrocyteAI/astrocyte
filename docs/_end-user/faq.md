...
In whatever storage backend you configure. Out of the box, Astrocyte uses an **in-memory store** (a Python dict) — fast for prototyping but not persistent. For production, configure a Tier 1 adapter like [pgvector](storage-backend-setup/#pgvector), [Qdrant](storage-backend-setup/#qdrant), [Neo4j](storage-backend-setup/#neo4j), or [Elasticsearch](storage-backend-setup/#elasticsearch). Memories are rows/vectors in your database, not in Astrocyte itself.

### Do I need a database to get started?

No. The default in-memory store works with zero setup — just `pip install astrocyte` and start calling `retain()` / `recall()`. You only need a database when you want persistence across restarts. See [Quick Start](quick-start/).

### What is a bank?

A bank is an isolated namespace for memories — like a folder. Each bank has its own access control, rate limits, and PII policies. Common patterns: one bank per user (tenant isolation), one per team (shared knowledge), or one per agent (task isolation). Banks are created automatically on first use. See [Bank management](bank-management/).

### Is Astrocyte an LLM gateway? An agent runtime?

Neither. Astrocyte is a **memory framework** — it sits between your agent and your storage backend. It does not route LLM traffic (that's LiteLLM, OpenRouter, etc.) and does not run agent loops (that's LangGraph, CrewAI, etc.). Astrocyte owns long-term memory, governed retrieval, and synthesis. See [How it works](how-it-works/).
...
All three use the same core and policy layer. See [Quick Start](quick-start/) for setup instructions.
...
Most teams start with pgvector. See [Storage backend setup](storage-backend-setup/).
...
The default config uses a **mock LLM** (returns empty embeddings). For real use, configure a provider like OpenAI, Anthropic, or any LiteLLM-compatible endpoint. See [Configuration reference](configuration-reference/).
...
This is enough to explore the API. For anything beyond local prototyping, configure at least a storage backend and LLM provider. See [Configuration reference](configuration-reference/).

### Can I use Astrocyte without Docker?

Yes. The library mode is just a Python package — `pip install astrocyte`. Docker is only needed for the standalone gateway deployment model. See [Quick Start](quick-start/).
...
To disable: omit the `barriers.pii` section from `astrocyte.yaml`, or set `pii_scan: false` on a per-bank basis. See [Configuration reference](configuration-reference/).
...
See [Bank management](bank-management/#multi-bank-queries).
...
This deletes all memories in the bank and logs the operation for audit. You can also delete by specific IDs, tags, or date range. See [Memory API reference](memory-api-reference/#forget----remove-memories).
...
Only if you configure a persistent storage backend (pgvector, Qdrant, Neo4j, Elasticsearch). The default in-memory store loses all data on restart. See [Storage backend setup](storage-backend-setup/).
...
Prometheus metrics are exposed per bank: memory count, storage bytes, retain/recall totals, recall latency. See [Monitoring & observability](monitoring-and-observability/).
