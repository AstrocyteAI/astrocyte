...
2. **Image attestations:** the publish workflow pushes **SLSA-style build provenance** to the registry (GitHub **artifact attestations**, Sigstore-backed). Verify after a release (needs [GitHub CLI](https://cli.github.com/) `gh`):
   ...
   ```bash
   gh attestation verify oci://ghcr.io/OWNER/REPO/astrocyte-gateway-py:v0.2.0 --repo OWNER/REPO
   ```
...
```bash
docker pull ghcr.io/astrocyteai/astrocyte/astrocyte-gateway-py:v0.2.0
docker run --rm -p 8080:8080 ghcr.io/astrocyteai/astrocyte/astrocyte-gateway-py:v0.2.0
