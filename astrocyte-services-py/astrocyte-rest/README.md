...
cd /path/to/astrocyte/astrocyte-services-py/astrocyte-rest && pip install -e .
ASTROCYTES_HOST=0.0.0.0 ASTROCYTES_PORT=8080 astrocyte-rest
```

Or:

```bash
uv run --directory astrocyte-services-py/astrocyte-rest astrocyte-rest
```
...
```bash
docker build -f astrocyte-services-py/astrocyte-rest/Dockerfile -t astrocyte-rest .
docker run --rm -p 8080:8080 astrocyte-rest
```
