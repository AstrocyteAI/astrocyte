"""Integration tests for PostgresStore against a real PostgreSQL + pgvector database."""
...
class TestStoreVectors:
    async def test_store_single_item(self, store: PostgresStore):
    ...
    async def test_upsert_overwrites(self, store: PostgresStore):
    ...
    async def test_dimension_mismatch_raises(self, store: PostgresStore):
...
class TestSearchSimilar:
    ...
    async def test_search_dimension_mismatch_raises(self, store: PostgresStore):
        with pytest.raises(ValueError, match="embedding_dimensions"):
...
class TestDelete:
    async def test_delete_existing(self, store: PostgresStore):
    ...
    async def test_delete_nonexistent_returns_zero(self, store: PostgresStore):
        deleted = await store.delete(["nope"], "bank-1")
        assert deleted == 0

    async def test_delete_empty_ids(self, store: PostgresStore):
...
class TestListVectors:
    async def test_pagination(self, store: PostgresStore):
    ...
    async def test_ordered_by_id(self, store: PostgresStore):
        await store.store_vectors([make_item("c"), make_item("a"), make_item("b")])
        items = await store.list_vectors("bank-1")
        assert [i.id for i in items] == ["a", "b", "c"]

    async def test_offset_beyond_end(self, store: PostgresStore):
        await store.store_vectors([make_item("v1")])
        result = await store.list_vectors("bank-1", offset=100)
        assert result == []

    async def test_empty_bank(self, store: PostgresStore):
...
class TestHealth:
    async def test_healthy(self, store: PostgresStore):
...
class TestMetadataAndTags:
    async def test_metadata_roundtrip(self, store: PostgresStore):
    ...
    async def test_null_metadata(self, store: PostgresStore):
    ...
    async def test_occurred_at_roundtrip(self, store: PostgresStore):
    ...
    async def test_retained_at_roundtrip(self, store: PostgresStore):
    ...
    async def test_delete_soft_forgets_current_rows(self, store: PostgresStore):
