...
async def stores(dsn: str):
    ...
    async with conn:
        ...
        await conn.execute("DELETE FROM astrocyte_source_chunks WHERE bank_id LIKE 'sar-%%'")
        await conn.execute("DELETE FROM astrocyte_source_documents WHERE bank_id LIKE 'sar-%%'")
        await conn.commit()
...
async def test_chunk_id_round_trips_through_postgres_store(stores) -> None:
    ...
    doc_id = await ss.store_document(
        SourceDocument(
    ...
    chunk_ids = await ss.store_chunks(
        [
            SourceChunk(id=f"{doc_id}:0", bank_id=bank, document_id=doc_id, chunk_index=0, text="first chunk text"),
            SourceChunk(id=f"{doc_id}:1", bank_id=bank, document_id=doc_id, chunk_index=1, text="second chunk text"),
        ]
    )
    ...
    await vs.store_vectors(
        [
            VectorItem(id="m1", bank_id=bank, vector=[1.0, 0.0, 0.0], text="first chunk text", chunk_id=chunk_ids[0]),
            VectorItem(id="m2", bank_id=bank, vector=[0.0, 1.0, 0.0], text="second chunk text", chunk_id=chunk_ids[1]),
        ]
    )
...
async def test_get_by_chunk_ids_returns_only_matching_vectors(stores) -> None:
    ...
    await vs.store_vectors(
        [
            VectorItem(id="m1", bank_id=bank, vector=[1.0, 0.0, 0.0], text="t1", chunk_id="c1"),
            VectorItem(id="m2", bank_id=bank, vector=[0.0, 1.0, 0.0], text="t2", chunk_id="c2"),
            VectorItem(
                id="m3", bank_id=bank, vector=[0.0, 0.0, 1.0], text="t3", chunk_id=None
            ),  # no provenance — must be excluded
        ]
    )
...
async def test_get_by_chunk_ids_isolates_by_bank(stores) -> None:
    ...
    await vs.store_vectors(
        [
            VectorItem(id="ma", bank_id=bank_a, vector=[1.0, 0.0, 0.0], text="a", chunk_id="shared-cid"),
            VectorItem(id="mb", bank_id=bank_b, vector=[1.0, 0.0, 0.0], text="b", chunk_id="shared-cid"),
        ]
    )
